...
void runInference() {
  ...
  // 1. Conv1: 輸入為 nn_input，輸出導向 buffer_A
  ...
  // 2. 【就地池化】：輸入是 buffer_A，輸出至 buffer_A 的後半段（偏移 1200 位元組）
  // pool1_out 降採樣結果安全存放在 buffer_A[1200 ~ 2399]
  maxpool1d((int8_t(*)[NN_CONV1_OUT_CH])buffer_A, (int8_t(*)[NN_CONV1_OUT_CH])(&buffer_A[1200]), NN_INPUT_LEN);

  // 3. 【Conv2 就地運算】：輸入讀取後半段 (&buffer_A[1200])，輸出覆蓋前半段 (buffer_A)
  // 兩塊區域完全錯開，不會發生讀寫衝突
  conv1d(&buffer_A[1200], buffer_A, CONV2_W, CONV2_B, NN_INPUT_LEN / 2, NN_CONV1_OUT_CH, NN_CONV2_OUT_CH, RQ_MULT_CONV2, RQ_SHIFT_CONV2);
  relu(buffer_A, (NN_INPUT_LEN / 2) * NN_CONV2_OUT_CH);

  // 4. 【後端記憶體疊加】：借用已經失去價值的後半段空檔（從 1200 開始切出安全暫存區）
  int8_t* gap_out = &buffer_A[1200];       // 借用 32 位元組
  int8_t* dense1_out = &buffer_A[1232];   // 借用 16 位元組
